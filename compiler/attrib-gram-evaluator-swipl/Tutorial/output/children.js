function root_radius ( _ale_arg0) { return 50 / _ale_arg0; }
///// pass /////
function visit_root_0(node) {
  ...
  (function () {
    ...
    for (var i = 0; i < children.length; i++) {
      var child = children[i]; 
      setAttribSafe(node, "radius", (50 / getAttribSafe(node, "childs_which_last") ));
      logger.log("         step radius: " + getAttribSafe(node, "radius"));
    }
  })();
  ...
  logger.log("    last init render: " + getAttribSafe(node, "render"));
    ...
    for (var i = 0; i < children.length; i++) {
      var child = children[i]; 
      setAttribSafe(node, "render", (paintOval(50 + getAttribSafe(child, "which") * getAttribSafe(node, "radius") * 2, 100, getAttribSafe(node, "radius"), getAttribSafe(node, "radius"), "#777")));
